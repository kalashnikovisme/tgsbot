...
func (m *MySQL) CreateStanduper(s *model.Standuper) (*model.Standuper, error) {
	res, err := m.conn.Exec(
		"INSERT INTO `standupers` (username, chat_id, language_code) VALUES (?, ?, ?)",
		s.Username, s.ChatID, s.LanguageCode,
	)
	if err != nil {
		return nil, err
	}
...
func (m *MySQL) UpdateStanduper(s *model.Standuper) (*model.Standuper, error) {
	m.conn.Exec(
		"UPDATE `standupers` SET username=?, language_code=? WHERE id=?",
		s.Username, s.LanguageCode, s.ID,
	)
